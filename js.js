...
// AUTH: Register / Log in + localStorage JSON
const USERS_KEY = 'lvlup_users_json';
const REGISTER_DRAFT_KEY = 'lvlup_register_draft_json';
const LOGIN_DRAFT_KEY = 'lvlup_login_draft_json';
...
function saveFormDraft(formId, key) {
  const form = document.getElementById(formId);
  if (!form) return;
  const data = {};
  form.querySelectorAll('input').forEach(input => { data[input.id] = input.value; });
  localStorage.setItem(key, JSON.stringify(data));
}
function loadFormDraft(formId, key) {
  const form = document.getElementById(formId);
  if (!form) return;
  try {
    const data = JSON.parse(localStorage.getItem(key)) || {};
    form.querySelectorAll('input').forEach(input => { if (data[input.id]) input.value = data[input.id]; });
  } catch(e) {}
}
function initAuth() {
  loadFormDraft('registerForm', REGISTER_DRAFT_KEY);
  loadFormDraft('loginForm', LOGIN_DRAFT_KEY);

  ['registerForm', 'loginForm'].forEach((id) => {
    const form = document.getElementById(id);
    if (!form) return;
    form.addEventListener('input', () => saveFormDraft(id, id === 'registerForm' ? REGISTER_DRAFT_KEY : LOGIN_DRAFT_KEY));
  });
  ...
  if (registerForm) registerForm.addEventListener('submit', (e) => {
    ...
    registerForm.reset();
    setAuthMessage('Cont creat cu succes! Datele au fost salvate în JSON/localStorage.', 'success');
  ...
  if (loginForm) loginForm.addEventListener('submit', (e) => {
    ...
    localStorage.setItem('lvlup_current_user_json', JSON.stringify(user));
